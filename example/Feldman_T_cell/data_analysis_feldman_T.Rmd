...
patient_IDs <- pbmc$patient_ID
patient_IDs <- as.matrix(patient_IDs)
rownames(patient_IDs) <- colnames(exp_data)
write.csv(patient_IDs, file=paste(filepath, 'patient_ids.csv', sep = ''))


```

#set weights for each cell
```{r}
...
table(pbmc$patient_ID)

patient_ID_freq <- as.data.frame(table(pbmc$patient_ID))
rownames(patient_ID_freq) <- patient_ID_freq$Var1
patient_ID_freq$Var1 <- NULL
patient_ID_freq$weight <- max(patient_ID_freq$Freq) / patient_ID_freq$Freq

cell_weights <- patient_ID_freq[pbmc$patient_ID, ]$weight
pbmc <- AddMetaData(pbmc, metadata = cell_weights, col.name = 'cell.weights')

sum(cell_weights[pbmc$responsive_status=='Non-responder'])
sum(cell_weights[pbmc$responsive_status=='Responder'])

phenotype_sum_weight1 = sum(cell_weights[pbmc$responsive_status=='Non-responder'])
phenotype_sum_weight2 = sum(cell_weights[pbmc$responsive_status=='Responder'])

cell_weights[pbmc$responsive_status=='Responder'] <- cell_weights[pbmc$responsive_status=='Responder'] * phenotype_sum_weight1 / phenotype_sum_weight2
...
# left one patient validation
```{r}
# DimPlot(object=pbmc, reduction='umap',  split.by = 'Conditions', group.by="patient_ID", pt.size = 1)

dataname = 'feldmanT'
result_path <- sprintf('./results/%s/R/%s/', dataname, expr_id)

scores_result_file = sprintf('./results/%s/py/%s/left_one_valid.csv', dataname, expr_id)
scores = read.csv(scores_result_file, header = T)

row.names(scores) = scores$X
scores$X <- NULL
colnames(scores) = c('left.one.predict.labels', 'left.one.confidence.score')

pbmc <- AddMetaData(object=pbmc, metadata=scores)
pbmc$left.one.predict.labels <- factor(pbmc$left.one.predict.labels, levels = levels(pbmc$pred_labels))

ggplot(data = pbmc@meta.data, mapping = aes(x=patient_ID, fill = left.one.predict.labels)) + geom_bar( position='stack') + facet_grid(~Conditions, scales="free_x") + scale_fill_manual(values = pal)+ theme(axis.text.x = element_text(angle = 60))
ggsave(sprintf('%sleft.one.pred_labels_count_in_patients.pdf', result_path), width = 7, height = 10)
