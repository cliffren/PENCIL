...
dataname <-  'PV14'
expr_id <- 'reg_cluster1_nonlinear(1over3)v2'
...
venn.plot <- venn.diagram(
    x = list(
        "Selected by Pencil" = row.names(rst)[rst$p.adj<0.05],
        "Before" = row.names(rst.raw)[rst.raw$p.adj<0.05]
    ),
    filename = NULL,
    col = "transparent",
...
ggsave(paste(result_path, 'overlap_of_Significantly_correlated_genes.png'), width = 10, height = 7)
...
# grid.draw(venn.plot)
plot_grid(venn.plot, labels = 'Significantly correlated genes', ncol = 1, nrow = 1)
ggsave(paste(result_path, 'overlap_of_Significantly_correlated_genes_with_deg.png'), width = 10, height = 7)
...
```{r}
# gene <- 'JUND'
gene <- setdiff(row.names(rst)[rst$p.adj<0.05], row.names(rst.raw)[rst.raw$p.adj<0.05])[1]
print(gene)
x = as.numeric(pbmc@meta.data[["Conditions"]])
y = t(pbmc@assays[["RNA"]]@scale.data)[, gene]
# l <- smooth.spline(x, y)
plot(x, y)
# lines(l, col='red')

x = pbmc_small$pred.time
y = t(pbmc_small@assays[["RNA"]]@scale.data)[, gene]
l <- smooth.spline(x, y)
plot(x, y)
lines(l, col='red')
```
